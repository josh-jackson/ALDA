...
This specifies the DGP and the family argument within `brms`

$$ \mu_i  = \beta \times Predictor_i $$
This specifies the model we are fitting. Here it is just a standard regression. 
...
$\beta   \sim \text{Normal}(0, 10)$
This says that we have a prior idea about the possible distribution of the regression coefficient. We think it is most likely zero, but we aren't that confident. As it could be higher or lower. 
...
## Differences in random effects with brms
...
```{r}
  brms::get_prior(cog ~ 0 + intercept + age_c + program + age_c:program + (1 + age_c | id), data = early_int_sim)
```

### Lets fit it again without that intercept. 
...
```{r}
  brms::get_prior(cog ~  1 + age_c + program + age_c:program + (1 + age_c | id), data = early_int_sim)
```

Is 102 the right prior we should have? 
```{r}
library(psych)
describe(early_int_sim$cog)
```



A few take aways: 1. not much has changed. Why? Because our data overwhelm the prior. Again, priors are not *that* big of a deal. 
2. We can forget about all of this if we do not use default priors. More on that later. 
...
Here we can change the prior to our regression coefficients
...
```{r}
plot(ex2.fit3.prior2)
```

So what priors should you use? 
...
As it turns out, we Bayesian use the log-likelihood (LL), too. Recall how the numerator in the right-hand side of Bayes’ Theorem was $p(\text{data} | \theta) p(\theta)$? That first part, $p(\text{data} | \theta)$, is the likelihood. In words, the likelihood is the *probability of the data given the parameters*. And we take the log of the likelihood rather than the likelihood itself because it’s easier to work with statistically. 
...
And if you wanted a more focused comparison, say between `ex2` and `ex2.fit2`, you'd just simplify your input.
...
Yet this approach neglects the $U$s. That is, it would be the same for everyone with COA = 1. We've been extracting the $U$s with `ranef()`. We also get them when we use `posterior_samples()`. Here we'll extract both the $\gamma$s as well as the $U$s for `id == 23`.
